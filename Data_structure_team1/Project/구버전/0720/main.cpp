...
void load(FILE *fp)
{
	...
	memberlist = (customer *)calloc(index, sizeof(customer));
	index = 0;
	fgets(buf, 256, fp);
	...
	while (true)
	{
		...
		memberlist[index].name = (char *)calloc(charcount + 1, sizeof(char));
		bufindex -= charcount;
		charcount = 0;
		...
		memberlist[index].address = (char *)calloc(charcount + 1, sizeof(char));
		bufindex -= charcount;
		charcount = 0;
		...
		memberlist[index].phone_num = (char *)calloc(charcount + 1, sizeof(char));
		bufindex -= charcount;
		charcount = 0;
		...
		memberlist[index].recommender = (char *)calloc(charcount + 1, sizeof(char));
		bufindex -= charcount;
		charcount = 0;
		...
		else
		{
			...
			memberlist[index].training.trainer_name = (char *)calloc(charcount + 1, sizeof(char));
			bufindex -= charcount;
			charcount = 0;
			while (buf[bufindex] != ',')
